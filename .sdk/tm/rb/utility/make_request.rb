...
module CoffeeTwoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CoffeeTwoResponse.new({})
    ...
    if fetch_err
      ...
      response = CoffeeTwoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CoffeeTwoResponse.new(fetched)
    else
