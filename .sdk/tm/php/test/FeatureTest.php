...
class FtCtrl extends CoffeeTwoControl
{
...
class FtClient
{
    ...
    private CoffeeTwoUtility $utility;

    public function __construct(CoffeeTwoUtility $utility, array $options)
    {
    ...
    public function get_utility(): CoffeeTwoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CoffeeTwoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CoffeeTwoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new CoffeeTwoUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (CoffeeTwoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CoffeeTwoConfig::make_feature($name);
    ...
    private static function buildUrl(CoffeeTwoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CoffeeTwoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CoffeeTwoResponse($fetched);
        }
    ...
    private function fail(CoffeeTwoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CoffeeTwoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
