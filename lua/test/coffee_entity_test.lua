...
describe("CoffeeEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set COFFEE_TWO_TEST_COFFEE_ENTID JSON to run live")
      return
    end
    ...
    local coffee_ref01_match_dt0 = {}
    local coffee_ref01_data_dt0_loaded, err = coffee_ref01_ent:load(coffee_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(coffee_ref01_data_dt0_loaded)
...
function coffee_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("COFFEE_TWO_TEST_COFFEE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["COFFEE_TWO_TEST_COFFEE_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["COFFEE_TWO_TEST_LIVE"] == "TRUE" then
  ...
  return {
    ...
    env = env,
    explain = env["COFFEE_TWO_TEST_EXPLAIN"] == "TRUE",
    live = live,
