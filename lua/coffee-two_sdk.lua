...
local CoffeeTwoSDK = {}
CoffeeTwoSDK.__index = CoffeeTwoSDK
...
CoffeeTwoSDK._make_feature = _make_feature


function CoffeeTwoSDK.new(options)
  local self = setmetatable({}, CoffeeTwoSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CoffeeTwoSDK:options_map()
...
function CoffeeTwoSDK:get_utility()
  return Utility.copy(self._utility)
end


function CoffeeTwoSDK:get_root_ctx()
  return self._rootctx
end


function CoffeeTwoSDK:prepare(fetchargs)
...
function CoffeeTwoSDK:direct(fetchargs)
...
-- by passing it.
function CoffeeTwoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CoffeeTwoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CoffeeTwoSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CoffeeTwoSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CoffeeTwoSDK.new(sdkopts)
  sdk.mode = "test"
