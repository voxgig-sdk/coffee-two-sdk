...
//     (\`new CoffeeTwoSDK(...)\` / \`CoffeeTwoSDK.test(...)\` become
//     \`CoffeeTwoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"coffee":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CoffeeTwoSDK(...)\` and any
// \`CoffeeTwoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CoffeeTwoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CoffeeTwoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
