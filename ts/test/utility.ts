...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.COFFEETWO_TEST_LIVE ||
    'TRUE' === process.env.COFFEETWO_TEST_OVERRIDE
  ) {
  ...
  m.COFFEETWO_TEST_EXPLAIN = process.env.COFFEETWO_TEST_EXPLAIN || m.COFFEETWO_TEST_EXPLAIN
