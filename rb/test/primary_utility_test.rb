...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CoffeeTwoOmni.make_runner(TEST_JSON_FILE, CoffeeTwoSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CoffeeTwoOmni.errify(err)
    end
