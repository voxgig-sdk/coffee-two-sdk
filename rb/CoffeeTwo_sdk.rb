...
require_relative 'CoffeeTwo_types'
...
class CoffeeTwoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CoffeeTwoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CoffeeTwoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CoffeeTwoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CoffeeTwoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CoffeeTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CoffeeTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CoffeeTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CoffeeTwoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CoffeeTwoError.new(
        "graphql_error", "CoffeeTwoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
