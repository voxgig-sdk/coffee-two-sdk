...
func NewCoffeeTwoSDK(options map[string]any) *CoffeeTwoSDK {
	sdk := &CoffeeTwoSDK{
		Mode:     "live",
		Features: []Feature{},
	}
...
func (sdk *CoffeeTwoSDK) OptionsMap() map[string]any {
...
func (sdk *CoffeeTwoSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *CoffeeTwoSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *CoffeeTwoSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *CoffeeTwoSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *CoffeeTwoSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *CoffeeTwoSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("CoffeeTwoSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *CoffeeTwoSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *CoffeeTwoSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("CoffeeTwoSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func (sdk *CoffeeTwoSDK) Coffee(data map[string]any) CoffeeTwoEntity {
	return NewCoffeeEntityFunc(sdk, data)
}



func TestSDK(testopts map[string]any, sdkopts map[string]any) *CoffeeTwoSDK {
	if sdkopts == nil {
