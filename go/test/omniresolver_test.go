...
import (
	...
	omni "github.com/voxgig-sdk/coffee-two-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CoffeeTwoError); is {
				out := map[string]any{
					"name":    "CoffeeTwoError",
...
func omniCtx(arg any, client *sdk.CoffeeTwoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CoffeeTwoSDK); is {
			client = live
