...
from coffeetwo_sdk.core.helpers import get_ctx_prop, to_map
...
class CoffeeTwoContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, CoffeeTwoSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CoffeeTwoResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CoffeeTwoResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return CoffeeTwoError(code, msg, self)
