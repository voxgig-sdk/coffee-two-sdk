...
class CoffeeEntity
{
    ...
    {
        if ($args) {
            $this->_data = CoffeeTwoHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = CoffeeTwoHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * Load a single Coffee.
     *
     * @param CoffeeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CoffeeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Coffee|array The loaded Coffee as an assoc-array at the
     *   SDK boundary; throws CoffeeTwoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CoffeeTwoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
