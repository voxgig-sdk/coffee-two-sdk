...
MCP server exposing the CoffeeTwo SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `coffee-two_list` | `entity`, optional `query` map | First page of records as JSON |
| `coffee-two_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `coffee-two_list` and
`coffee-two_load` tools become available in new sessions.
